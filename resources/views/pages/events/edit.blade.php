...
<body class="hold-transition sidebar-mini layout-fixed layout-navbar-fixed layout-footer-fixed">
    <div class="content-wrapper"> <br>
      <div class="container-fluid">
        <div class="card">
              <b>
                <h5 class="text-center ">Modification de <span class="text-bold">{{$event->titre}}</span> </h5>
              ...
              <form class="border p-4" action="{{route('events.update',$event)}}" method="post" enctype="multipart/form-data">
                ...
                <div class="form-group">
                  ...
                  <select class="form-control custom-select @error('categories_id') is-invalid @enderror" name="categories_id" value="{{ old('categories_id') }}" >
                    <option >choisir une categorie</option>
                    @foreach($categories as $categorie)  
                        <option value="{{$categorie->id}} " {{$event->categories_id == $categorie->id ? 'selected' : ''}}>{{$categorie->nom}}</option>
                    @endforeach
                  </select>
                  @error('categories_id')
                      <span class="invalid-feedback" role="alert">
                ...
                <div class="form-group green-border-focus">
                  ...
                  <textarea class="form-control @error('description') is-invalid @enderror"   placeholder=" " id="description" value="{{ old('description')  ?? $event->titre }}" rows="3" type="text" name="description"></textarea>
                        @error('description')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
